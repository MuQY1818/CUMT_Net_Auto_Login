...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                             QLabel, QLineEdit, QComboBox, QPushButton, QCheckBox, QMessageBox, QFrame)
...
class AutoLoginApp(QMainWindow):
    ...
    def initUI(self):
        ...
        close_button.setStyleSheet("""
            QPushButton { background-color: transparent; color: white; font-size: 18px; font-weight: bold; border: none; }
        ...
        for label_text, widget_attr in [
            ('学号:', ('student_id_input', QLineEdit())),
            ('密码:', ('password_input', QLineEdit())),
            ('运营商:', ('operator_input', QComboBox()))
        ]:
            layout = QHBoxLayout()
            ...
            if isinstance(widget, QLineEdit):
                widget.setStyleSheet("""
                    QLineEdit { border: 1px solid #CCCCCC; border-radius: 4px; padding: 5px; background-color: #F5F5F5; }
                ...
                widget.setStyleSheet("""
                    QComboBox { border: 1px solid #CCCCCC; border-radius: 4px; padding: 5px; padding-right: 20px; background-color: #F5F5F5; }
                    QComboBox::drop-down { subcontrol-origin: padding; subcontrol-position: top right; width: 20px; border-left: 1px solid #CCCCCC; border-top-right-radius: 3px; border-bottom-right-radius: 3px; }
                    QComboBox::down-arrow { image: none; border-left: 5px solid transparent; border-right: 5px solid transparent; border-top: 5px solid #333333; width: 0; height: 0; margin-right: 5px; }
                """)
        ...
        self.login_button.setStyleSheet("""
            QPushButton { background-color: #4CAF50; color: white; border: none; border-radius: 4px; padding: 10px; font-size: 16px; }
    ...
    def setAutoStart(self, enable):
        ...
        try:
            with reg.OpenKey(key, key_path, 0, reg.KEY_ALL_ACCESS) as registry_key:
                if enable:
                    reg.SetValueEx(registry_key, app_name, 0, reg.REG_SZ, f'"{sys.executable}" "{os.path.abspath(__file__)}" --auto-start')
                else:
    ...
    def login(self):
        ...
        ip_attempts = ["10.4.238.172", "", "0.0.0.0", "255.255.255.255"]
    ...
    def verify_login(self):
        try:
            ...
            if "已登录" in response.text or "注销" in response.text:
                return True
